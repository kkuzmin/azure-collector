...
const HTTP_CLIENT_RETRY_COUNT = 7;
...
class O365Management extends msRestAzure.AzureServiceClient {
    constructor(credentials, officeTenantId, options) {
        ...
        options.filters.push(new msRest.ExponentialRetryPolicyFilter(HTTP_CLIENT_RETRY_COUNT));
    ...
    subscriptionsContent(contentType, startTs, endTs, options, callback) {
        ...
        if (endTs && entTs != '') queryParameters.push('endTime=' + encodeURIComponent(endTs));
        if (queryParameters.length > 0) {
            requestUrl += '?' + queryParameters.join('&');
        }
